...
# 用法：bash .claude/scripts/github-manage/push-to-github.sh ["提交说明"]
...
API_CONFIG_REL="skills/nano-banana-generation/references/api-config.md"
API_CONFIG_FILE="$REPO_ROOT/$API_CONFIG_REL"
...
# ── 脱敏 api-config.md 用于提交（先解除 skip-worktree）─────────────────────
git update-index --no-skip-worktree "$API_CONFIG_REL" 2>/dev/null || true

# 保存本地 API Key 真实值
_API_STATUS_VAL=$(grep "^API_STATUS=" "$API_CONFIG_FILE" | sed 's/^API_STATUS=//')
_API_KEY_VAL=$(grep "^API_KEY=" "$API_CONFIG_FILE" | sed 's/^API_KEY=//')

# 写入脱敏版本（API_KEY 清空）
cat > "$API_CONFIG_FILE" << API_SANITIZED
# Nano Banana API 配置

## 状态
API_STATUS=$_API_STATUS_VAL
...
## 说明
# API_STATUS 可选值：
#   not_configured — 未配置，首次使用时引导
#   configured     — 已配置，可直接调用
#
# API Key 获取地址：https://apimart.ai/console/token
# API Key 通过 push 脚本脱敏上传，本地真实值受 git skip-worktree 保护
API_SANITIZED

# ── 暂存所有变更（包含脱敏后的 github-config.sh + api-config.md）────────────
echo "→ 暂存变更..."
git add .
...
REAL2
  ...
  # 恢复 api-config.md 本地真实值
  cat > "$API_CONFIG_FILE" << API_REAL2
# Nano Banana API 配置
...
# API Key 通过 push 脚本脱敏上传，本地真实值受 git skip-worktree 保护
API_REAL2
  git update-index --skip-worktree "$API_CONFIG_REL" 2>/dev/null || true
  echo ""
...
# ── 推送完成：恢复 api-config.md 本地真实值 ──────────────────────────────────
cat > "$API_CONFIG_FILE" << API_REAL
# Nano Banana API 配置
...
echo "  github-config.sh / api-config.md 已自动脱敏上传，本地真实配置已恢复"
echo "=================================================="
